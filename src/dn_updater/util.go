	...
	for _, v := range etcds {
		...
		if err != nil {
			...
			if err != nil {
				...
				/* clear escape */
				// resolve nested escape
				buffer.escape_json = bytes.Replace(buffer.raw_json, []byte("\\\\"), []byte(""), -1)
				buffer.escape_json = bytes.Replace(buffer.escape_json, []byte("\\\""), []byte("\""), -1)
				buffer.escape_json = bytes.Replace(buffer.escape_json, []byte("\\n"), []byte(""), -1)
				buffer.escape_json = bytes.Replace(buffer.escape_json, []byte("\"{"), []byte("{"), -1)
				buffer.escape_json = bytes.Replace(buffer.escape_json, []byte("}\""), []byte("}"), -1)
				if opts.debug {
